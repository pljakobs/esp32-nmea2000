...
bool GwDalyBleClient::connect(const std::string &macAddr) {
    ...
    (void)macAddr;
    if (logger_ != nullptr) {
        logger_->logDebug(GwLog::ERROR, "Daly: BLE support not available (BLEDevice.h missing)");
    }
    connected_ = false;
    return false;
#endif
...
size_t GwDalyBleClient::sendRequest(const uint8_t *request, size_t requestLen,
    ...
    if (!txChar_->writeValue((uint8_t *)request, requestLen, true)) {
        LOG_DEBUG(GwLog::LOG, "Daly: BLE write failed");
        return 0;
    }
