    ...
    while (true) {
        ...
        if (!bleClient.isConnected()) {
            ...
            if (bleClient.connect(batteryMac.c_str())) {
                ...
                LOG_DEBUG(GwLog::LOG, "daly: BLE connect failed, retry in %dms", reconnectDelay);
        ...
        if (responseLen == 0) {
            ...
            LOG_DEBUG(GwLog::LOG, "daly: poll timeout or BLE error");
        ...
        if (!GwDalyProtocol::decodePackDataResponse(responseBuf, responseLen, packData)) {
            ...
            LOG_DEBUG(GwLog::LOG, "daly: failed to decode response");
            continue;
        }
